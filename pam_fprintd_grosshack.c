          ...
          if (r == 0)
            {
              ...
              if (has_recieved_sigusr1)
                {
                  if (debug)
                    pam_syslog (data->pamh, LOG_DEBUG, "Got SIGUSR1: assuming pw received");
                  return PAM_AUTHINFO_UNAVAIL;
                }
...
prompt_pw (void *d)
{
  ...
  pam_prompt (data->pamh, PAM_PROMPT_ECHO_OFF, &pw, "Place finger on fingerprint reader or enter password: ");
